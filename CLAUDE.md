...
Scripts: `npm run dev` · `build` · `preview` (= `next start`) · `lint` · `scrape:backfill` · `scrape:daily` · `normalize:images` · `normalize:directions` · `verify:images` · `flip:image`.
...
| `scripts/standardize-directions.mjs` | Deterministic toe-direction detector + flipper (`npm run normalize:directions`). **Heuristic — unreliable on rocker/high-stack runners; do not trust it as the source of truth.** |
| `scripts/verify-directions.mjs` | **Orientation GATE** (`npm run verify:images`): builds labeled contact sheets in `.image-audit/` (running shoes first = high risk) and blocks (exit 1) until you eyeball them and pass `-- --ack`. Deliberately does not auto-guess direction. |
...
- **Shoe images** (`shoeImages`) point to **self-hosted, normalized** files in `public/shoes/*.png` — each trimmed to the shoe and bottom-aligned on a 16:10 white canvas by `npm run normalize:images`, so cards (`object-fit: contain`) share one baseline with no clipping. All shoes face one way (**toe-left**). `normalize:images` auto-orients via a heel-height heuristic, **but that heuristic is unreliable and has silently shipped toe-right shoes and even mis-flipped correct ones** — it systematically inverts modern rocker / high-stack runners (their forefoot stack reads as the "heel"). **Therefore orientation is not decided by code: after any image change you MUST run the gate `npm run verify:images`, open the `.image-audit/` contact sheets, confirm every shoe faces toe-left, flip offenders with `npm run flip:image <slug>`, then `npm run verify:images -- --ack`.** Do not trust the auto-orient or any single signal/agent — eyeball it. To add/replace: set a clean **white/transparent-background, side-profile** source URL (via the `shoe-image-fixer` agent), then `npm run normalize:images`, then the gate. A non-white/colored source background won't trim and will fill the tile — keep sources clean. Don't hand-write `/shoes/` paths.
